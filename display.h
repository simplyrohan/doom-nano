...
// Adafruit_ST7735 display = Adafruit_ST7735(TFT_CS, TFT_DC, TFT_RST, TFT_SCLK, TFT_MISO);
...
#else
  display.drawPixel(x, y, color ? ST7735_WHITE : ST7735_BLACK);
#endif
}
...
// ST7735 is optimized to draw a full line at once with drawFastVLine
void drawVLine(uint8_t x, int8_t start_y, int8_t end_y, uint8_t intensity)
{
  int8_t y;
...
#ifdef OPTIMIZE_ST7735
  for (int d = 0; d < RES_DIVIDER; d++)
  {
#ifdef COLOR_DISPLAY
    int color;
    switch (intensity)
    ...
    display.drawFastVLine(x + d, lower_y, higher_y - lower_y + 1, color);

#else
    display.drawFastVLine(x + d, lower_y, higher_y - lower_y + 1, intensity ? ST7735_WHITE : ST7735_BLACK);
#endif
  }
#else
#ifdef OPTIMIZE_SSD1306
        ...
        for (uint8_t ox = 0; ox < pixel_size; ox++)
        {
          for (uint8_t oy = 0; oy < pixel_size; oy++)
          {
            ...
            if (sprite == 3)
            {
              ...
              // drawPixel(x + tx + ox, y + ty + oy, ST7735_RED, true);
              display.drawPixel(x + tx + ox, y + ty + oy, ST7735_RED);
            }
            else
            {
