...
void renderMap(const uint8_t level[], double view_height)
{
  ...
  for (uint8_t x = 0; x < SCREEN_WIDTH; x += RES_DIVIDER)
  {
    ...
    {
      display.drawFastVLine(x + d, 0, RENDER_HEIGHT, ST7735_BLACK);
    }
...
void renderGun(uint8_t gun_pos, double amount_jogging)
{
  ...
  {
    // Gun fire
    display.drawBitmap(x + 6, y - 11, bmp_fire_bits, BMP_FIRE_WIDTH, BMP_FIRE_HEIGHT, 0xFFFF);
  }
  ...
  display.drawBitmap(x, y, bmp_gun_bits, BMP_GUN_WIDTH, clip_height, ST7735_WHITE);
}
...
void loop(void)
{
  ...
  {
    fadeScreen(i, 0);
    // display.display(); // Not needed for st7735
    delay(40);
  }
  exit_scene = false;
}
